    ...
    function createComment(){
        ...
        $.post("functions/createNewsComment.php", {storyID:$storyID, comment:$comment, userID:$userID, guestName:$name, responseID:$response, thread: 0}, function(data){
            if(data != "false"){
               ...
               var newPost = '<div class="post" user_id="">';
               newPost +=   '<div class="picMask"><img src="' + $("#profilePic").attr("src") + '"></div>';
               newPost +=   '<div>';
               
               newPost += '<p><strong class="userRollover" user_id="">'+$("#userBlock h2").text()+'</strong> <span class="mainText"><span class="newsPost">commented on the story <a href="http://vancitysocial.ca/news.php?story='+$storyID+'">'+$thisTitle+'</a>...</span> '+$comment+'</span></p>';
